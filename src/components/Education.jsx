...
function CertCard({ cert, delay }) {
  return (
    <div
      className="fade-up bg-bg-card border border-border glow-border rounded-xl p-5 flex items-center gap-4"
    ...
    >
      ...
      <div className="min-w-0">
        <p className="text-white text-sm font-medium leading-snug truncate">{cert.name}</p>
        <p className="text-muted text-xs mt-0.5">{cert.issuer}</p>
      </div>
      <span className="font-mono text-xs text-accent-3 ml-auto shrink-0">{cert.year}</span>
    </div>
  )
}
